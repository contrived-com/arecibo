...
- All timestamps sent or received by the transponder must use RFC 3339 date-time format in UTC.
...
- Keep per-service integration minimal:
  - copy transponder artifacts into image
  - use canonical `entrypoint.sh`
  - set a small set of `TRANSPONDER_*` env vars
- No shared app base image requirement.
- Heterogeneous Dockerfiles are expected and supported.

## Telemetry Architecture Direction

- App emits telemetry locally to the transponder using a best-effort interface.
- Preferred local ingest is Unix socket `SOCK_DGRAM` (no network dependency).
- If the transponder is absent/unavailable, app should continue without blocking.
- The transponder owns sampling/filtering/redaction and uplink behavior based on homebase policy.
- The transponder can run independently of app telemetry and still provide announce/heartbeat.
- In `GO_DARK`, the transponder stays alive and keeps local ingest behavior stable while dropping outbound sends.

## Reliability and Safety

- Delivery is at-least-once; duplicates are expected and tolerated.
- Transponder queues must be bounded; never risk unbounded disk growth in default mode.
- Resource usage should be observable (memory/process stats in heartbeats).
- The transponder should run lower priority than app (`nice` and optional `ionice`).
